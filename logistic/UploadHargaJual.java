...
public class UploadHargaJual extends ERPWebPage
{
	/**
	...
	private class FileListView extends ListView<File>
	{
		...
		protected void populateItem(ListItem<File> listItem)
		{
			...
			listItem.add(new Link<Void>("import")
			{
				@Override
				public void onClick()
				{
					...
					try
					{
						doingImport(file.getAbsolutePath());
						info("Imported " + file);
					}
					catch(Exception e)
					{
			...
			if (uploads != null)
			{
				for (FileUpload upload : uploads)
				{
					...
					try
					{
						...
						UploadHargaJual.this.info("saved file: " + upload.getClientFileName());
					}
					catch (Exception e)
					{
	...
	private boolean doingImport(String filename) throws AxisFault
	{
		Service _service=new Service(Service.INVENTORY_SERVICE_URL);
		Object[] params=new Object[]{filename};
    	
    	Class[] retTypes =new Class[]{Boolean.class};
    	try {
			Object[] response=_service.callServiceInventory("importHargaJual", params,retTypes);
	...
	/** Log. */
	private static final Logger log = LoggerFactory.getLogger(UploadHargaJual.class);
	 ...
	 *            Page parameters
	 */
	public UploadHargaJual(final PageParameters parameters)
	{
	...
	{
		return ((ERPApplication)Application.get()).getUploadFolder();
	}
}