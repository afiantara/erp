...
public class ApprovalPRequest extends WebPage{
	...
	private static ApprovalPRequest app;
	...
	public static ApprovalPRequest get()
	{
		return app;
	}
	
	public ApprovalPRequest(PageParameters p,PRHeader ph)
	{
		this(ph);
		readOnly(false);
	...
	public ApprovalPRequest()
	{
		this(new PRHeader());
		
	}
	
	public ApprovalPRequest(PRHeader ph)
	{
		...
		add(new AjaxLink<Group1>("approved")
		{
			/**
			 * 
			 */
	...
	public boolean approvePRequest()
	{
		form.process(null);
		...
		item.setTglupdate(Long.parseLong(dateNow));
		item.setUserupdate(UserInfo.USERID);
		item.setApprovedby(UserInfo.USERID);
		item.setApprovedtgl(dateNow);
